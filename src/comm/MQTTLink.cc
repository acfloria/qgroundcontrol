...
static bool contains_target(const QList<MQTTClient*> list, const QHostAddress& address, quint16 port)
{
    for(MQTTClient* target: list) {
...
MQTTLink::MQTTLink(SharedLinkConfigurationPointer& config)
    ...
    , _running(false)
    , _socket(NULL)
    , _udpConfig(qobject_cast<MQTTConfiguration*>(config.data()))
    , _connectState(false)
{
    if (!_udpConfig) {
...
bool MQTTLink::_isIpLocal(const QHostAddress& add)
{
...
void MQTTLink::_writeBytes(const QByteArray data)
{
    ...
    for(MQTTClient* target: _udpConfig->targetHosts()) {
    ...
    for(MQTTClient* target: _sessionTargets) {
        _writeDataGram(data, target);
    }
}

void MQTTLink::_writeDataGram(const QByteArray data, const MQTTClient* target)
{
...
void MQTTLink::readBytes()
{
    ...
    while (_socket->hasPendingDatagrams())
    {
        ...
        if(!contains_target(_sessionTargets, asender, senderPort)) {
            ...
            MQTTClient* target = new MQTTClient(asender, senderPort);
...
bool MQTTLink::_hardwareConnect()
{
    ...
    if (_connectState) {
        ...
        QObject::connect(_socket, &QUdpSocket::readyRead, this, &MQTTLink::readBytes);
        emit connected();
    } else {
...
void MQTTLink::_registerZeroconf(uint16_t port, const std::string &regType)
{
#if defined(QGC_ZEROCONF_ENABLED)
...
void MQTTLink::_deregisterZeroconf()
{
#if defined(QGC_ZEROCONF_ENABLED)
    if (_dnssServiceRef)
    {
...
void MQTTConfiguration::_copyFrom(LinkConfiguration *source)
{
    MQTTConfiguration* usource = dynamic_cast<MQTTConfiguration*>(source);
    if (usource) {
        _localPort = usource->localPort();
        _clearTargetHosts();
        for(MQTTClient* target: usource->targetHosts()) {
            if(!contains_target(_targetHosts, target->address, target->port)) {
                MQTTClient* newTarget = new MQTTClient(target);
...
void MQTTConfiguration::_clearTargetHosts()
{
 ...
 */
void MQTTConfiguration::addHost(const QString host)
{
    // Handle x.x.x.x:p
    if (host.contains(":"))
    {
...
void MQTTConfiguration::addHost(const QString& host, quint16 port)
{
    ...
    if(ipAdd.isEmpty()) {
        ...
        if(!contains_target(_targetHosts, address, port)) {
            MQTTClient* newTarget = new MQTTClient(address, port);
            _targetHosts.append(newTarget);
            _updateHostList();
        }
    }
...
void MQTTConfiguration::removeHost(const QString host)
{
    if (host.contains(":"))
    {
        ...
        for(int i = 0; i < _targetHosts.size(); i++) {
            MQTTClient* target = _targetHosts.at(i);
            if(target->address == address && target->port == port) {
...
void MQTTConfiguration::setLocalPort(quint16 port)
{
    _localPort = port;
}
...
void MQTTConfiguration::_updateHostList()
{
    _hostList.clear();
    for(int i = 0; i < _targetHosts.size(); i++) {
        MQTTClient* target = _targetHosts.at(i);
