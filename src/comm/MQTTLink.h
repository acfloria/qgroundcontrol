...
class MQTTClient {
public:
    MQTTClient(const QHostAddress& address_, quint16 port_)
        : address(address_)
        , port(port_)
    {}
...
public:
    ...
    const QList<MQTTClient*> targetHosts() { return _targetHosts; }
    ...
    QString     settingsTitle        () { return tr("MQTT Link Settings"); }
...
private:
    ...
    void    _writeDataGram          (const QByteArray data, const MQTTClient* target);
    ...
    bool                    _running;
    QUdpSocket*             _socket;
    MQTTConfiguration*       _udpConfig;
    bool                    _connectState;
    QList<MQTTClient*>       _sessionTargets;
